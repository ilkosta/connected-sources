...
import org.connected_sources.api.dto.onboarding.ApprovedOnboardingView;
import org.connected_sources.api.dto.onboarding.OnboardingRequestCreate;
...
//    this.tenantRepo = tenantRepo;
  ...
  public ResponseEntity<OnboardingView> request(@RequestBody OnboardingRequestCreate in) {
...
//    if (repo.existsIdem(idem)) {
//      // idempotent “duplicate”
//      OnboardingView view = repo.findSummaryByNaturalKey(in.producerName(),in.email())
//              .map(this::toView)
//              .orElse(new OnboardingView(-1L, OnboardingState.REQUESTED, in.producerName(), in.email()));
//      return ResponseEntity.accepted().body(view);
//    }
    ...
    try {
      ...
      vars.put("requesterUserId", in.requesterUserId());
...
//      vars.put("requesterEmail",    in.email());
      ...
      contactRepo.curatorsPrimaryEmail().forEach(c -> {
        ...
        notifier.enqueue(
                ...
                EventType.ONBOARDING_REQUESTED, false
        );
      });
  ...
  public ResponseEntity<ApprovedOnboardingView> approve(@PathVariable long id) {
    ...
    Function<OnboardingSummary,OnboardingSummary> sendApprovation = (OnboardingSummary s) -> {
      ...
      var vars = new java.util.HashMap<String, Object>();
      vars.put("requesterUserId", requester.user_id());
      ...
      String reqeusterMsg = "È stata inoltrata a" + s.email() + " il modulo di registrazione a Connected Sources.";

      var rendered = templateService.render(NotificationTemplate.ONBOARDING_APPROVED.name(), vars, null);


      notifier.enqueue(NotificationTemplate.ONBOARDING_APPROVED,
              requester.email(), Channel.EMAIL,
              "[ registration request approved ] " + s.producerName(),
              reqeusterMsg, Duration.ofHours(24),
              EventType.ONBOARDING_ACCEPTED, false);
  ...
  public ResponseEntity<OnboardingView> reject(@PathVariable long id) {
    var ctx = TenantContextHolder.get();

    try {
      repo.transitionState(id, OnboardingState.REJECTED, ctx.userId(), Map.of());
    } catch (JsonProcessingException e) {
      throw new RuntimeException(e);
    }
    ...
    return repo.findSummary(id)
               ...
               .map(ResponseEntity::ok)
               .orElseGet(() -> ResponseEntity.notFound().build());
//    return ResponseEntity.ok(new OnboardingView(id, OnboardingState.REJECTED));
  }
